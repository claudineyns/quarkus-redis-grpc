#!/usr/bin/env bash
# Smoke test dos contadores (INCR/INCRBY) contra o proxy implantado, via
# counter.py. Limpa demo:counter:*, regenera os stubs (o proto mudou), abre
...
echo "limpando chaves demo:counter:* no Redis..."
...
oc --context "$OC_CXT" -n "$REDIS_NAMESPACE" exec "$REDIS_POD" -- \
  redis-cli del demo:counter:n demo:counter:text >/dev/null
...
"$PY" "${CLIENT_DIR}/counter.py" localhost:18080
