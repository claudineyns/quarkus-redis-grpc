#!/usr/bin/env bash
# Smoke do HashService — fatia 1, via hashes.py. O próprio cliente semeia (HSET) e
# limpa (HDEL) o estado; regenera stubs, abre port-forward e roda o cliente.
set -euo pipefail
cd "$(dirname "$0")"
source ./env.sh
...
"$PY" "${CLIENT_DIR}/hashes.py" localhost:18080
