# CLAUDE.md — Guia mandatório do projeto `redis-grpc`

> Este documento é a fonte de verdade para decisões de arquitetura e implementação.
...
- **Governança a partir daqui:** este projeto (e este CLAUDE.md) permanece a
  fonte de verdade que **administra** o projeto adjacente — em especial o
  **contrato `.proto`**, que é definido e versionado AQUI (papel análogo ao
  WSDL/XSD compartilhado em SOAP). A extensão consumirá esse contrato; não o
  redefine.
...
> **Cursor:** `SCAN`/`HSCAN`/`SSCAN` usam iteração por cursor — modelados como
> RPC unário com cursor no request e `{ próximo cursor + página }` no response,
> espelhando o protocolo Redis 1:1.
>
...
## 11. Premissas de operação (ambiente de desenvolvimento)
...
- **`README.md` sempre em inglês.** O conteúdo do README é mantido exclusivamente
  em inglês (este CLAUDE.md permanece em português).

### 11.1 Modo de trabalho: discutir antes de implementar (MANDATÓRIO)
...
- [ ] Formato de propagação de erros Redis → gRPC.
