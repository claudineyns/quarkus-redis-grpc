#!/usr/bin/env bash
# Smoke test do GETDEL contra o proxy implantado, via getdel.py. Semeia o
# estado, regenera os stubs, abre port-forward e roda o cliente (que também
# confirma a exclusão da chave com um GET subsequente).
set -euo pipefail
cd "$(dirname "$0")"
source ./env.sh
...
echo "semeando estado demo:getdel:* no Redis..."
...
oc --context "$OC_CXT" -n "$REDIS_NAMESPACE" exec "$REDIS_POD" -- sh -c '
  redis-cli del demo:getdel:present demo:getdel:absent demo:getdel:hash >/dev/null
  redis-cli set demo:getdel:present v >/dev/null
  redis-cli hset demo:getdel:hash f v >/dev/null
'
...
"$PY" "${CLIENT_DIR}/getdel.py" localhost:18080
