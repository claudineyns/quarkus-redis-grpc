#!/usr/bin/env bash
# Smoke do SetService — fatia 2 (SPOP), via setpop.py. O próprio cliente semeia
# (SADD) e exercita SPOP; regenera stubs, abre port-forward e roda o cliente.
set -euo pipefail
cd "$(dirname "$0")"
source ./env.sh
...
"$PY" "${CLIENT_DIR}/setpop.py" localhost:18080
